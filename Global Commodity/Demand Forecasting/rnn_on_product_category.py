...
def main():
  ...
  rnnmodel(args.num_epoches, args.lr, args.fc_n, args.fc_stride, args.fc_start, args.train_len, args.hidden_dim, args.dropout, args.batch_size, args.max_lag, args.trading_file)


def rnnmodel(e, lr, fcn, fcsrd, fcsrt, tl, hdd, d, b, ml, f):
  ...
  def fit_it(model, train, val, fcsrt):
      ...
      print("\nbeginning the training of the {0} RNN:".format("Vanilla"))
      ...
      print("training of the {0} RNN has completed:".format("RNN"), f'{res_time:.2f} sec')
  ...
  def run_RNN(series, train, val, ml, tl, hdd, b, e, d, lr,fcsrt):
      periodicity = check_seasonal(series, ml)
      cov_train, cov_val, covariates=cov_series(series, fcsrt)
      # set the model up
      model_RNN = RNNModel(
          model="RNN",
          model_name="Vanilla" + str(" RNN"),
      ...
      # fit the model
      fit_it(model_RNN, train, val, fcn)

      # compute N predictions
      pred = model_RNN.predict(n=fcn, future_covariates=covariates) 

      # plot predictions vs actual
      plot_fitted(pred, series, "RNN")
  ...
  pred_trf = [run_RNN(series_trf, train_trf, val_trf, ml, tl, hdd, b, e, d, lr, fcsrt)]
  metrics(pred_trf, df)
